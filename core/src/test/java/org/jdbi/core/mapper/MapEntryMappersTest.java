...
class MapEntryMappersTest {

    @Test
    void withersDoNotMutateReceiver() {
        MapEntryMappers base = new MapEntryMappers();
        assertThat(base.getKeyColumn()).isNull();
        assertThat(base.getValueColumn()).isNull();

        MapEntryMappers derived = base.keyColumn("k").valueColumn("v");
