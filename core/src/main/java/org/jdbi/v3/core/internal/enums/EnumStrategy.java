...
public interface EnumStrategy extends ArgumentFactory, ColumnMapperFactory {
    ...
    <E extends Enum<E>> ColumnMapper<E> getMapper(Class<E> enumClass);
    ...
    default Optional<ColumnMapper<?>> build(Type type, ConfigRegistry config) {
        return getEnumType(type).map(this::getMapper);
    }

    @SuppressWarnings("unchecked")
    static <E extends Enum<E>> Optional<Class<E>> getEnumType(Type type) {
