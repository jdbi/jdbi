...
public final class ConfigRegistry {
    ...
    private Function<ConfigRegistry, JdbiConfig<?>> configFactory(Class<? extends JdbiConfig<?>> configClass) {
        return configFactories.computeIfAbsent(configClass, ConfigRegistry::createConfigFactory);
    }

    private static <T extends JdbiConfig<?>> Function<ConfigRegistry, JdbiConfig<?>> createConfigFactory(Class<T> klass) {
        ...
        try {
            ...
            throw new IllegalStateException("Unable to use constructor taking ConfigRegistry to create " + klass, e);
        }
        try {
            MethodHandle mh = MethodHandles.lookup().findConstructor(klass,
            ...
            IllegalStateException failure = new IllegalStateException("Unable to instantiate config class " + klass
