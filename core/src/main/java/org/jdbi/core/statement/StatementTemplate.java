...
public class StatementTemplate<R> {
    final StatementTemplateBuilder builder;
    ...
    StatementTemplate(final StatementTemplateBuilder builder, final ResultSetScanner<ResultIterable<R>> scanner) {
        ...
        this.parsedSql = stmtConfig.getSqlParser()
            .parse(renderedSql, StatementContext.create(config, null, StatementTemplate.class));
    }

    public StatementTemplateBinding<R> with(final Handle handle) {
