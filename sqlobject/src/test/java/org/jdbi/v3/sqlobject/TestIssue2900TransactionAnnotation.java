 ...
 */
public class TestIssue2900TransactionAnnotation {
    ...
    @Test
    public void annotatedWriteCommitsThroughTheCustomHandler() {
        jdbi.useExtension(Dao.class, dao -> dao.insertSerializable(1, "apple"));

        assertThat(countingHandler.count()).isEqualTo(1);

        int rows = jdbi.withHandle(h -> h.attach(Dao.class).countRows());
    ...
    public interface Dao {
        ...
        @Transaction(TransactionIsolationLevel.SERIALIZABLE)
        void insertSerializable(@Bind("id") int id, @Bind("name") String name);
    }
