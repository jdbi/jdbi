...
[source,java]
----
public class UserIdListColumnMapper implements ColumnMapper<List<UserId>> {
    @Override
    public List<UserId> map(ResultSet rs, int col, StatementContext ctx) throws SQLException {
        final Integer[] userIds = (Integer[]) rs.getArray(col).getArray();
...
List<UserId> userIds = handle.createQuery("select user_ids from groups where id = :id")
      .bind("id", 1)
      .map(new UserIdListColumnMapper())
      .findOnly();
----
