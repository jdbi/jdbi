...
* The transaction callbacks (`afterCommit`, `afterRollback`) run at each explicit commit or rollback. A commit that fails rolls the transaction back and runs the `afterRollback` callbacks.
