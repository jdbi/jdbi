...
:kotlinsqldocs: ./apidocs-kotlin/jdbi3-kotlin-sqlobject/jdbi3-kotlin-sqlobject/org.jdbi.v3.
:jdkdocs: https://docs.oracle.com/en/java/javase/17/docs/api
...
Jdbi runs on all Java versions 17 or later.
All releases are built with the latest LTS version of Java using Java 17 bytecode compatibility.

[IMPORTANT]
Jdbi ended support for Java 11 with version 3.50, and Java 8 with version 3.39.
There may be occasional backports of security relevant things or major bugs, if necessary.
...
The most convenient way to get access to a handle is by using the link:{jdbidocs}/core/Jdbi.html#withHandle(org.jdbi.v3.core.HandleCallback)[withHandle^] or link:{jdbidocs}/core/Jdbi.html#useHandle(org.jdbi.v3.core.HandleConsumer)[useHandle^] methods on the link:{jdbidocs}/core/Jdbi.html[Jdbi^] class.
...
Any handle that is passed in through a callback (link:{jdbidocs}/core/Jdbi.html#withHandle(org.jdbi.v3.core.HandleCallback)[Jdbi#withHandle()^], link:{jdbidocs}/core/Jdbi.html#useHandle(org.jdbi.v3.core.HandleConsumer)[Jdbi#useHandle()^], link:{jdbidocs}/core/Jdbi.html#inTransaction(org.jdbi.v3.core.HandleCallback)[Jdbi#inTransaction()^], link:{jdbidocs}/core/Jdbi.html#useTransaction(org.jdbi.v3.core.HandleConsumer)[Jdbi#useTransaction()^]) is managed through Jdbi:
...
* link:{jdbidocs}/core/mapper/PropagateNull.html[@PropagateNull^] on a class or an attribute maps a `null` value instead of the whole object when a specific column is null.
See <<propagate-null,Using `@PropagateNull`>>.
...
The link:{jdbidocs}/core/mapper/PropagateNull.html[@PropagateNull^] annotation tells a reflection mapper to return `null` instead of an object when a specific column is null.
A typical use is a nested object from a `LEFT OUTER JOIN`.
When the join finds no row, the mapper returns `null` instead of an object with only null attributes.

Put the annotation on the class or on an attribute:

* On the class, set the value to the name of the column to examine.
This column does not need to map to an attribute of the class.
For example, it can be a foreign key.
For the `ConstructorMapper`, the annotation with a value can also be on the factory method.
* On an attribute, do not set a value.
The mapper examines the value that the column mapper returns for this attribute.
For the `BeanMapper`, put the annotation on the getter or the setter.
For the `KotlinMapper`, put it on a constructor parameter or a `var` member property.

The column name on the class is relative to the prefix of the mapper, and does not include that prefix.
The prefix comes from `@Nested("prefix")`, or from a mapper that you make with a prefix, such as `ConstructorMapper.of(User.class, "receiver")`.
Jdbi puts the prefix in front of the name (`sender.id`), and the default column name matchers match this name to the `sender_id` column.
When a mapper with a prefix maps a `@Nested` attribute, the two prefixes add up.
As a result, you can map the same class with different prefixes, and two times in the same row, as the next example shows.

If no column matches the name, the annotation has no effect, and Jdbi does not show an error.
Thus, `@PropagateNull("sender_id")` under the `sender` prefix does nothing, because Jdbi looks for `sender.sender_id`.
If you enable strict matching with `ReflectionMappers.setStrictMatching(true)`, the examined column must also map to an attribute.
Jdbi does not count the examined column as a matched column.

This example maps a transfer with a sender and an optional receiver:

[source,java,indent=0]
----
include::{exampledir}/PropagateNullTest.java[tag=types]
----

The query names the columns of the two users `sender_id`, `sender_name`, `receiver_id`, and `receiver_name`.
When the transfer has no receiver, the `receiver_id` column is null, and the `receiver` attribute is `null`:

[source,java,indent=0]
----
include::{exampledir}/PropagateNullTest.java[tag=nested]
----

A mapper that you make with a prefix uses the same rule:

[source,java,indent=0]
----
include::{exampledir}/PropagateNullTest.java[tag=prefix]
----

Because the mapper examines the value that the column mapper returns, an `Optional<String>` attribute on a SQL `NULL` column is `Optional.empty()`, which is not `null`, and does not propagate.
For a primitive attribute such as `int id`, the `BeanMapper`, `ConstructorMapper`, and `FieldMapper` also treat a SQL `NULL` as `null`.
On a `@Nested` attribute, the mapper examines the nested object: if the nested object is `null`, the parent object is also `null`.
...
The link:{jdbidocs}/core/result/ResultIterable.html#withStream(org.jdbi.v3.core.result.StreamCallback)[withStream()^] and link:{jdbidocs}/core/result/ResultIterable.html#useStream(org.jdbi.v3.core.result.StreamConsumer)[useStream()^] methods pass the Stream into a callback.
...
The link:{jdbidocs}/core/result/ResultIterable.html#withStream(org.jdbi.v3.core.result.StreamCallback)[withStream()^] method allows passing a result back to the caller, link:{jdbidocs}/core/result/ResultIterable.html#useStream(org.jdbi.v3.core.result.StreamConsumer)[useStream()^] only executed the code in the callback.
...
link:{jdbidocs}/core/result/JoinRowReducer.html[JoinRowReducer^] is a `RowReducer` for join queries, so that most of them need no hand-written reducer.
You name the root type, then declare each relation with its column prefix, its type and the method that links it to the parent.

[source,java,indent=0]
----
List<Contact> contacts = handle.createQuery(
        "SELECT c.id, c.name, p.id phone_id, p.number phone_number "
            + "FROM contacts c LEFT JOIN phones p ON p.contact_id = c.id")
    .reduceRows(JoinRowReducer.of(Contact.class).mappedBy(BeanMapper::of) // <1>
        .many("phone", Phone.class, Contact::addPhone)) // <2>
    .toList();
----

<1> The root type reads its columns without a prefix, and its key column is `id`. `of(Contact.class, "email")` names another key column.
`mappedBy(BeanMapper::of)` creates a bean mapper for every type and prefix in the reducer, so nothing is registered.
Without it, mappers come from the registry by type and prefix: the root takes the mapper registered for `Contact` without a prefix, a custom one or `BeanMapper.factory(Contact.class)`, and the phones take `BeanMapper.factory(Phone.class, "phone")`.
<2> `many()` declares a to-many relation under the column prefix `phone`, so a phone reads `phone_id` and `phone_number`.
Each distinct `phone_id` produces one `Phone`, and the linker adds it to its contact once.
A row with a `NULL` `phone_id`, as produced by a `LEFT JOIN` without a match, adds nothing.

`one()` declares a to-one relation.
One type is one table: every object is identified by its type and its key, so a table row is one instance no matter through how many relations the query reaches it.
Here the author and the editor of a book are one `Person` instance when they are the same person, and an author shared by several books is one instance:
...
List<Book> books = handle.createQuery(
        "SELECT b.id, b.title, a.id author_id, a.name author_name, e.id editor_id, e.name editor_name "
            + "FROM books b LEFT JOIN persons a ON a.id = b.author_id LEFT JOIN persons e ON e.id = b.editor_id")
...
Relations can be combined freely.
A query that joins two collections produces the cross product of the two collections for each root, and `many()` adds each joined object to a root only once:

[source,java,indent=0]
----
List<Book> books = handle.createQuery(SELECT_BOOKS_WITH_CHAPTERS_AND_TAGS)
    .reduceRows(JoinRowReducer.of(Book.class).mappedBy(BeanMapper::of)
        .one("author", Person.class, Book::setAuthor)
        .many("chapter", Chapter.class, Book::addChapter)
        .many("tag", Tag.class, Book::addTag))
    .toList();
----

A joined type can have relations of its own.
Pass a reducer for the type instead of the class, and prefixes compound: an award of the author reads `author_award_id` and `author_award_name`.
A class that is declared anywhere in the reducer refers to that declaration, so the editor below is the same `Person` node as the author, awards included:

[source,java,indent=0]
----
List<Book> books = handle.createQuery(SELECT_BOOKS_WITH_AUTHORS_AND_AWARDS)
    .reduceRows(JoinRowReducer.of(Book.class).mappedBy(BeanMapper::of)
        .one("author", JoinRowReducer.of(Person.class)
                .many("award", Award.class, Person::addAward),
            Book::setAuthor)
        .one("editor", Person.class, Book::setEditor))
    .toList();
----

The root type is declared too, so a self join names it by class, and the parent is the same instance as the root category with that key.
The recursion ends where the selected columns end: without a `parent_parent_id` column there is no grandparent.

[source,java,indent=0]
----
List<Category> categories = handle.createQuery(
        "SELECT c.id, c.name, pc.id parent_id, pc.name parent_name "
...
A type may take its mapper from another factory than the root, for example `JoinRowReducer.of(Tag.class).mappedBy(ConstructorMapper::of)` for a record.
Linkers run once per distinct pair after the last row, and the relations of a joined type are linked before the joined type is linked to its parent, so a linker receives a complete object unless the graph is recursive.
If the query selects a type on several paths, the object is mapped from the shallowest path, so the result does not depend on row order.
Root objects are returned in the order of their first row, the objects of a `many()` relation are linked to a parent in the order of their first row with it, and only keys in the root key column become roots.
Keys are read with `RowView.getColumn(String)`, which returns the value as the JDBC driver returns it, and compared by value: integral keys of any JDBC type are equal when their values are, decimals ignore trailing zeros, and a `byte[]` key is compared by content.
A relation is present on a path when the result set has its key column there, so select the root's own foreign key columns under another label than a relation key, or not at all.
With `SELECT b.*` and a relation `category`, the book's `category_id` column alone would produce a `Category` that has only its key.
`one()` links each parent once and fails if a second, different key appears for the same parent, which shows that the join is not to-one.
A reduction also fails if the result set has no root key column, or if a declared relation has its key column on no path, which catches a misspelled alias.
These column checks run on the first row, so a result set with no rows passes them.
A `JoinRowReducer` is immutable and holds no state between reductions, so one instance can serve many statements.
...
* The link:{jdbidocs}/core/Handle.html#inTransaction(org.jdbi.v3.core.HandleCallback)[inTransaction^] and link:{jdbidocs}/core/Handle.html#useTransaction(org.jdbi.v3.core.HandleConsumer)[useTransaction^] methods on link:{jdbidocs}/core/Handle.html[Handle^] objects open a transaction on the handle itself and then pass it to the callback code.
...
In addition to the link:{jdbidocs}/core/transaction/LocalTransactionHandler.html[standard transaction handler^], Jdbi includes a link:{jdbidocs}/core/transaction/CMTTransactionHandler.html[handler that works correctly in a container managed environment^], a link:{jdbidocs}/core/transaction/NoOpTransactionHandler.html[handler that ignores transactions completely^] for connections that an external framework manages, and a link:{jdbidocs}/core/transaction/SerializableTransactionRunner.html[serializable transaction handler^] that allows multiple concurrent operations on a single handle to retry transparently.
...
=== Transactions managed outside Jdbi

Some applications manage transactions outside of Jdbi.
A connection pool can disable autocommit for every connection, a container can own the transaction, or a framework such as Spring can wrap the data source in a transaction-aware proxy.
Jdbi supports three strategies for these connections: the handle joins the external transaction (with the default handler, or with `CMTTransactionHandler` in a container), Jdbi ignores transactions completely, or Jdbi takes ownership and manages the transactions itself.

==== Joining a transaction: connections with autocommit disabled

The default transaction handler treats a connection that arrives with autocommit disabled as enrolled in a transaction that the connection owner manages.
The handle joins this transaction:

* link:{jdbidocs}/core/Handle.html#isInTransaction()[isInTransaction()^] returns `true` for the life of the handle.
* `inTransaction` and `useTransaction` run the callback in the open transaction and do not commit. The connection owner commits. This is the same rule that applies to a nested `inTransaction` call inside an explicit `begin()`.
* The variants that take an isolation level throw a `TransactionException` when the requested level is different from the level of the connection. `TransactionIsolationLevel.UNKNOWN` joins without this check.
* `begin()` does nothing.
* Explicit `commit()` and `rollback()` operate on the open transaction. After a commit or rollback, the next statement starts a new transaction, and the cycle can repeat on the same handle.
* The transaction callbacks (`afterCommit`, `afterRollback`) run at each explicit commit or rollback. A commit that fails rolls the transaction back and runs the `afterRollback` callbacks. A rollback that fails runs them too. When the connection commits and only the autocommit restore afterwards fails, the `afterCommit` callbacks run.
* `close()` does not commit, roll back, or throw. The connection returns to its owner with its transaction state unchanged.

[NOTE]
Because `inTransaction` and `useTransaction` never commit on such a connection, transaction retry handlers such as `SerializableTransactionRunner` do not engage.
To let Jdbi manage transactions on these connections, use `LocalTransactionHandler.managed()` (see below).

==== Joining a container transaction

link:{jdbidocs}/core/transaction/CMTTransactionHandler.html[CMTTransactionHandler^] is for container managed (J2EE CMT) environments.
It never begins or commits.
When autocommit is disabled on the connection, the handle reports an open transaction and callbacks join it.
A call to `rollback()` throws a `TransactionException`, so that the exception propagates to the container, which then rolls the transaction back.

==== Ignoring transactions completely

link:{jdbidocs}/core/transaction/NoOpTransactionHandler.html[NoOpTransactionHandler^] never reads or changes the transaction state of the connection.
Use it when the connection is wrapped by a proxy, for example the Spring `TransactionAwareDataSourceProxy` or an XA data source, where the autocommit flag does not show the real transaction state:

* `begin()` and `commit()` do nothing. The external manager starts and commits the transaction.
* `rollback()` throws a `TransactionException`, so that the external manager sees the failure and rolls the transaction back.
* `isInTransaction()` returns `false` and does not examine the connection.
* `inTransaction` and `useTransaction` run the callback directly.
...
==== Letting Jdbi manage transactions with autocommit disabled

Some connection pools disable autocommit for every connection as a precaution, and the application still wants Jdbi to manage the transactions.
For this configuration, use link:{jdbidocs}/core/transaction/LocalTransactionHandler.html#managed()[LocalTransactionHandler.managed()^]:

[source,java]
----
Jdbi jdbi = Jdbi.create(dataSourceWithAutocommitDisabled);
jdbi.setTransactionHandler(LocalTransactionHandler.managed());
----

The managed handler does not read the autocommit flag to find the transaction state.
It tracks the transactions that Jdbi demarcates:

* `inTransaction` and `useTransaction` start a transaction, run the callback, and commit. When the callback throws, they roll the transaction back.
* `isInTransaction()` returns `true` only between `begin()` (or the start of a transaction callback) and the commit or rollback.
* The variants that take an isolation level work. Retry handlers engage when composed with the managed handler: `new SerializableTransactionRunner(LocalTransactionHandler.managed())`.
* A statement executed outside of a transaction is not committed, and `commit()` or `rollback()` outside of a transaction does not touch the connection.
JDBC does not define what happens to the uncommitted work when the connection closes: most drivers and pools roll it back, and the Oracle driver documents an implicit commit.
To discard the work explicitly, roll back on the JDBC connection.
* `close()` on a handle with an open transaction rolls the transaction back and throws, the same as with autocommit enabled.

[NOTE]
JDBC has one transaction per connection.
When a statement executes outside of a Jdbi transaction and a Jdbi transaction then commits on the same connection, the commit includes the earlier statement.
On PostgreSQL, the open transaction also rejects a change of the isolation level, so `useTransaction(TransactionIsolationLevel.SERIALIZABLE, ...)` fails on that handle until the transaction ends.
This is a property of JDBC and applies to all strategies on this page.
...
Jdbi registers the most common element types out of the box: the primitive and boxed
types, `String`, `UUID`, enums, and the `java.time` types `LocalDate`,
`LocalTime`, `LocalDateTime`, `OffsetDateTime`, `OffsetTime`, `Instant`, and
`ZonedDateTime` (the last two are bound as the equivalent `OffsetDateTime`).

[NOTE]
Database plugins may register additional array element types or adjust how arrays are bound.
`PostgresPlugin` registers `Duration` and `Period` (bound as `interval`), and binds the date-bearing `java.time` element types as text literals that cover the full Postgres date range, including BC dates.
...
For most join queries, a <<JoinRowReducer>> does the same work without a hand-written `accumulate()` method.
`@UseRowReducer` requires a class with a public no-arg constructor, so extend `JoinRowReducer` and pass the configured reducer to the superclass constructor.
The registered mappers are looked up by type and prefix, so the query aliases the parent folder columns as `parent_id` and `parent_name`, and the document columns as `document_id` and so on:
...
public class FolderJoinReducer extends JoinRowReducer<Folder> {
...
Similar to the link:{jdbidocs}/core/Jdbi.html#inTransaction(org.jdbi.v3.core.HandleCallback)[Jdbi#inTransaction()^] and link:{jdbidocs}/core/Jdbi.html#useTransaction(org.jdbi.v3.core.HandleConsumer)[Jdbi#useTransaction()^] operations in the core API, transactions are not nested.
...
There are a number of additional JUnit 5 extensions in the `jdbi-core` test artifact.
...
It provides helpers for writing JUnit 4 tests integrated with a managed database instance.
This makes writing unit tests quick and easy!
You must remember to include the database dependency itself, for example to get a pure H2 Java database:
...
* Generic JDBC database support - link:{jdbidocs}/testing/junit5/JdbiGenericExtension.html[JdbiGenericExtension^].
...
* https://testcontainers.org/[Testcontainers^] - link:{jdbidocs}/testing/junit5/tc/JdbiTestcontainersExtension.html[JdbiTestcontainersExtension^].
...
The link:{jdbidocs}/testing/junit5/JdbiH2Extension.html[JdbiH2Extension^] class allows for a few additional customizations:
...
* link:{jdbidocs}/testing/junit5/JdbiPostgresExtension.html[JdbiPostgresExtension^] manages a local database on the host.
...
Jdbi supports JDBC type databases with the link:{jdbidocs}/testing/junit5/tc/JdbiTestcontainersExtension.html[JdbiTestcontainersExtension^]:
...
[TIP]
The `dbContainer` object is registered as class level field for sharing between all the tests in the test class https://junit.org/junit5/docs/current/user-guide/#extensions-registration-programmatic-static-fields[as described in the JUnit 5 documentation^].
...
The link:{jdbidocs}/testing/junit5/tc/JdbiTestcontainersExtension.html[JdbiTestcontainersExtension^] has built-in support to isolate each test method in a test class.
...
The link:{jdbidocs}/testing/junit5/tc/TestcontainersDatabaseInformation.html#of(java.lang.String,java.lang.String,java.lang.String,java.util.function.BiFunction)[TestcontainersDatabaseInformation#of()^] method allows creation of custom database information instances which then can be passed into link:{jdbidocs}/testing/junit5/tc/JdbiTestcontainersExtension.html#instance(org.jdbi.v3.testing.junit5.tc.TestcontainersDatabaseInformation,org.testcontainers.containers.JdbcDatabaseContainer)[JdbiTestcontainersExtension#instance(TestcontainersDatabaseInformation, JdbcDatabaseContainer)^] to create extensions with custom behavior.
...
Ensure the Kotlin compiler's https://kotlinlang.org/docs/reference/using-maven.html#attributes-specific-for-jvm[JVM target version^] is set to at least 17:
...
The *jdbi3-mysql* plugin provides enhanced integration with the
...
----
dependencies {
    implementation("org.jdbi:jdbi3-mysql:{project_version}")
}
----
...
The deprecated `spring5` module supports Spring Framework version 5.x and 6.0. These versions are no longer under OSS support and updates are no longer publicly available.

[NOTE]
The spring5 module will be removed in a future version of Jdbi!
Please migrate to the `spring` module.
...
[NOTE]
The Jdbi plugin supports vavr 1.0.x.
It does not support the vavr 1.0.0-alpha releases, which are binary incompatible with vavr 1.0.
...
link:{jdbidocs}/core/Jdbi.html#useExtension(java.lang.Class,org.jdbi.v3.core.extension.ExtensionConsumer)[Jdbi#useExtension()^] methods are used to access SQLObject functionality:
...
link:{jdbidocs}/core/extension/ExtensionFactory.html#attach(java.lang.Class,org.jdbi.v3.core.extension.HandleSupplier)[ExtensionFactory#attach()^] method with the current extension type and a handle supplier.
...
link:{jdbidocs}/core/extension/ExtensionFactory.html#attach(java.lang.Class,org.jdbi.v3.core.extension.HandleSupplier)[ExtensionFactory#attach()^] method immediately.
...
They must be stateless and can only modify or operate on the link:{jdbidocs}/core/extension/ExtensionHandler.html[ExtensionHandler^] object that was passed into the link:{jdbidocs}/core/extension/ExtensionHandlerCustomizer.html#customize(org.jdbi.v3.core.extension.ExtensionHandler,java.lang.Class,java.lang.reflect.Method)[ExtensionHandlerCustomizer#customize()^]
method.
...
In general, Jdbi's separate artifacts each provide a single relevant plugin (e.g. `jdbi3-sqlite`), and such modules will be auto-loadable.
Modules that provide no (e.g. `jdbi3-commons-text`) or multiple (e.g. `jdbi3-core`) plugins typically will not be.

[TIP]
The developers encourage you to install plugins explicitly.
Code with declared dependencies on the module it uses is more robust to refactoring and provides useful data for static analysis tools about what code is or is not used.
...
interface.
By calling link:{jdbidocs}/core/config/Configurable.html#setTemplateEngine(org.jdbi.v3.core.statement.TemplateEngine)[Configurable#setTemplateEngine()^] method on the link:{jdbidocs}/core/Jdbi.html[Jdbi^], link:{jdbidocs}/core/Handle.html[Handle^], or any SQL statement like link:{jdbidocs}/core/statement/Update.html[Update^] or link:{jdbidocs}/core/statement/Query.html[Query^], the template engine used to render SQL can be changed:
...
A template engine may implement the link:{jdbidocs}/core/statement/TemplateEngine.html#parse(java.lang.String,org.jdbi.v3.core.config.ConfigRegistry)[TemplateEngine#parse()^] methods to create an intermediary representation of a template that can be used to render a template faster.
...
The generator also registers a link:{jdbidocs}/sqlobject/GeneratedSqlObjectProvider.html[GeneratedSqlObjectProvider^] for each generated class through the `ServiceLoader`.
Jdbi uses the provider to create instances and to find the extension methods without reflection.
Generated SQL objects therefore work in a <<GraalVM Native Image>> without reachability metadata.
Classes generated by a Jdbi version before 3.55.0 have no provider. Recompile them with the current generator.
...
SQL objects that use the <<generator, code generator>> need no reachability metadata.
The generator registers each implementation with the `ServiceLoader`, which native-image supports without configuration.
...
link:{jdbidocs}/core/Jdbi.html#useTransaction(org.jdbi.v3.core.HandleConsumer)[useTransaction^], or link:{jdbidocs}/core/Jdbi.html#inTransaction(org.jdbi.v3.core.HandleCallback)[inTransaction^].
