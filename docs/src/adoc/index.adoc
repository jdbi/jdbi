...
While Java is the primary target for Jdbi, we provide modules for some languages that allow more idiomatic access to Jdbi functionality.
...
[CAUTION]
Other languages don't always emit equivalent idiomatic Java code in the way you expect.
Especially, by default annotations on Kotlin or Lombok properties may not be copied to the Java property.
This means you may need to configure Lombok annotation copying like
`lombok.copyableAnnotations += org.jdbi.v3.json.Json`
or make sure Kotlin attaches annotations to generated Java properties like `@get:Json`.
...
In particular, if an argument in the first batch is `null` then Jdbi may guess an "unknown" type which
is not appropriate for a future non-`null` argument. Consider using
...
This is very flexible but when binding a large link:{jdbidocs}/core/statement/PreparedBatch.html[PreparedBatch^] it incurs a serious performance penalty
...
When mapping a value `@Nested Optional<MyBean>` then it will wrapped inside an optional, and will be `Optional.empty()`
when `MyBean` itself would be null (e.g. because `@PropagateNull` annotation is used on a `MyBean` field that was null).
...
By using the `TypeResolvingCodecFactory` from the link:{jdbidocs}/guava/package-summary.html[guava^] module, it is possible to use codecs that are registered for subclasses or interface types for concrete classes. This is necessary to e.g. map https://github.com/google/auto/blob/main/value/userguide/index.md[Auto Value^] generated classes to database columns.
...
Annotate a SqlObject type, or a method or field within, with `@Definition` to define an attribute
for all SqlObject methods of the type. The key will default to the field or method name. The value
will default to the result of calling the method or getting the field.
...
An Update or link:{jdbidocs}/core/statement/PreparedBatch.html[PreparedBatch^] may automatically generate keys. These keys
...
[TIP]
The `dbContainer` object is registered as class level field for sharing between all the tests in the test class https://junit.org/junit5/docs/current/user-guide/#extensions-registration-programmatic-static-fields[as described in the JUnit 5 documentation^]. Some IDEs warn that this field should not be static. *This is a false positive/bug*. Changing `dbContainer` to an instance field will result in starting and stopping a new database per test which is a significant performance penalty.
...
- Isolation type _Catalog_: A new catalog (or database) is created for each test. The schema name is not used or the database does not support schemas.
...
When active, Jdbi will record a `jdbi.statement` event for every statement executed, indicating the timestamp,
duration, executed sql, and parameters.
...
from the `jdbi3-opentelemetry` artifact will cause Jdbi statements to emit trace spans recording data
similarly as JFR events above. Additionally, it will add the trace id to JFR events.
...
Jdbi supports Kotlin value classes as long as an appropriate mapper for the underlying data type has been registered. For value types that wrap standard data types such as `int`, `String` or `UUID`, this is normally the case.
...
The `spring` module supports Spring 6.1 (Spring Boot 3.2.x) and newer. All versions of Spring framework that are under OSS support are supported. Other versions may work but are not tested and bug reports for older versions will not be addressed.
...
classpath. In the String template group file, the `selectAll` group will be located and used for rendering.
...
Here, since the fully qualified class name is `com.foo.AccountDao`,
SQL will be rendered by loading the `selectAll` template from the `com/foo/AccountDao.sql.stg` String template group file on the  classpath.
...
[NOTE]
The link:{jdbidocs}/stringtemplate4/UseStringTemplateEngine.html[@UseStringTemplateEngine^] annotation activates parsing of the value of a SQL method annotation using the String Template engine while the link:{jdbidocs}/stringtemplate4/UseStringTemplateSqlLocator.html[@UseStringTemplateSqlLocator^] uses string template group files on the class path to load and parse templates. These annotations are mutually exclusive and the behavior, if these annotations are used simultaneously, is undefined.
...
In this example, since the fully qualified class name is `com.foo.AccountDao`,
SQL will be loaded from the file `com/foo/AccountDao/selectAll.sql.ftl` on the
classpath. By default, every template must be a separate file on the class path.
...
[NOTE]
The link:{jdbidocs}/freemarker/UseFreemarkerEngine.html[@UseFreemarkerEngine^] annotation activates parsing of the value of a SQL method annotation using the Freemarker engine while the link:{jdbidocs}/freemarker/UseFreemarkerSqlLocator.html[@UseFreemarkerSqlLocator^] uses files on the class path to load and parse FreeMarker templates. These annotations are mutually exclusive and the behavior, if these annotations are used simultaneously, is undefined.
...
provides an extension factory which does not use proxy objects and returns these as extension type implementations. Calling a method on the extension type will invoke the methods on the generated classes directly.
