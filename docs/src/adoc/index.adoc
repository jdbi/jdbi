...
link:https://search.maven.org/search?q=g:org.jdbi%20AND%20v:{project_version}[Maven Central^],
...
it will go away. We are already no longer support all tests on JDK 8 (only 11+) and require JDK 11 or
better to build the code base (but still maintain JDK 8 byte code compatibility).

In order to run on JDK 8, some default dependencies, such as `caffeine` need to be explicitly version
managed to a version that is supported on JDK 8.
...
[CAUTION]
Our `org.jdbi.*.internal` packages are not considered public API and they are not maintained or changed in a backwards compatible way.
...
If your operation returns some result, use
