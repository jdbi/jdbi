...
As with Core, we often need to apply configuration to a SQL method or an entire
SQL object. For this we use Configuring Annotations--i.e. annotations that
apply some configuration.

Jdbi provides many configuring annotations out of the box:

* The
  link:{jdbidocs}/sqlobject/config/package-summary.html[org.jdbi.v3.sqlobject.config^]
  package contains the most commonly used annotations, including those for registering
  mappers and arguments, and for configuring SQL statement rendering and parsing.
* The
  link:{jdbidocs}/sqlobject/locator/package-summary.html[org.jdbi.v3.sqlobject.locator^]
  package contains annotations which configure Jdbi to load the SQL statement for a
  SQL method from some alternate source, e.g. a file on the classpath.
* The
  link:{jdbidocs}/stringtemplate4/package-summary.html[org.jdbi.v3.stringtemplate4^]
  package contains annotations to configure Jdbi to load SQL from StringTemplate 4
  `.stg` files on the classpath, and/or to parse SQL templates using the
  ST4 template engine.

You can also create your own configuring annotations. Let's look at an example
from Jdbi to see how it's done.

First, create an annotation that you want to attach configuration to:
...
----
<1> All configuring annotations should have a `RUNTIME` retention policy.
<2> Configuring annotations may target types, methods, or both. Typically,
placing an annotation on the type means "apply this annotation to each method."

TIP: Strictly speaking, the `@Target` annotation is not required, but it's a
good practice to include it, so that annotations can only be applied where they
...
public class UseTemplateEngineImpl implements Configurer {
  ...
  public void configureForMethod(ConfigRegistry registry,
                                 ...
                                 Class<?> sqlObjectType,
                                 Method method) {
    configure(registry, (UseTemplateEngine) annotation);
  }
  ...
  private void configure(ConfigRegistry registry,
                         UseTemplateEngine annotation) {
    try {
      TemplateEngine templateEngine = annotation.value().getConstructor().newInstance();
      registry.get(SqlStatements.class).setTemplateEngine(templateEngine);
    }
    catch (NoSuchMethodException e) {
      throw new RuntimeException("Cannot construct " + anno.value(), e);
    }
  }
}
----

In this example, we're applying the same configuration, whether the
`@UseTemplateEngine` annotation is used on the SQL object type or method.
However, configurers are free to apply different configurations depending on
whether the annotation was placed on the type or the method.

For configuring annotations with only one target, (e.g. `@KeyColumn` and
...
annotation to your configuring annotation (yo dawg), to tell Jdbi that
`UseTemplateEngineImpl` is responsible for implementing the behavior of
`@UseTemplateEngine`.
...
Your annotation is now ready to use in any SQL object:

[source,java]
----
public interface DynamicDao {
  @UseTemplateEngine(StringTemplateEngine.class)
  @SqlQuery("select <columns; separator=\", \"> from <table>")
  public <T> List<T> list(@MapTo Class<T> type,
                          @Define String table,
...
Some configurations only make sense to apply to individual SQL statements, like
parameter bindings, cursor direction, or result fetch size. For these
situations, Jdbi provides several annotations:

* The
  link:{jdbidocs}/sqlobject/customizer/package-summary.html[org.jdbi.v3.sqlobject.customizer^]
  package contains the most commonly used customizing annotations, including
  those for binding parameters, defining attributes, and controlling the fetch
  behavior of the statement's result set.
* The
  link:{jdbidocs}/sqlobject/statement/package-summary.html[org.jdbi.v3.sqlobject.statement^]
  package contains the `@MapTo` annotation (seen in the previous section). This
  annotation is used on a method parameter, to dynamically specify the mapped
  type at the time the method is invoked.
* The
  link:{jdbidocs}/sqlobject/transaction/package-summary.html[org.jdbi.v3.sqlobject.transaction^]
  package contains the `@TransactionIsolation` annotation, which (predictably)
  sets the transaction isolation level of the method.
* The
  link:{jdbidocs}/jpa/package-summary.html[org.jdbi.v3.jpa^]
  package contains the `@BindJpa` annotation, for binding properties to column
  according to JPA `@Column` annotations.

Customizing annotations are applied only after the
link:{jdbidocs}/core/statement/SqlStatement.html[SqlStatement^] has been
created.

As with <<Configuring Annotations>>, you can create your own SQL statement
customizing annotations and attach runtime behavior to them.
...
<1> All statement customizing annotations should have a `RUNTIME` retention
    policy.
<2> Customizing annotations may target types, methods, parameters, or any
    combination of them. Placing a customizing annotation on a type typically
    means "apply this customization to every method." When used on parameters,
    often the annotation will use the argument passed to the method instead of
    some value on the annotation.
...
Statement customizing annotations are a little bit different from configuring
annotations. The `SqlStatementCustomizerFactory` produces two different types
of "statement customizer" command objects:
...
<2> link:{jdbidocs}/sqlobject/customizer/SqlStatementCustomizer.html[SqlStatementCustomizer^]
    ...
    void.
<3> Since the customization is the same at the method level as at the type
    level, we simply delegate to the type-level method for brevity.
<4> link:{jdbidocs}/sqlobject/customizer/SqlStatementParameterCustomizer.html[SqlStatementParameterCustomizer^]
    can also be implemented as a lambda, but instead it accepts a `SqlStatement`
    and the value that was passed into the method on the annotated parameter.

Finally, add the
link:{jdbidocs}/sqlobject/customizer/SqlStatementCustomizingAnnotation.html[@SqlStatementCustomizingAnnotation^]
annotation the customizing annotation, to tell Jdbi that `MaxRowsFactory` is
responsible for implementing the behavior of `@MaxRows`:
...
In practice, you will likely get better database performance by using a `LIMIT`
clause in your SQL statement than by using `@MaxRows`.
