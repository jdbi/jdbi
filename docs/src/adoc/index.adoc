...
Jdbi supports three strategies for these connections: the handle joins the external transaction, Jdbi ignores transactions completely, or Jdbi takes ownership and manages the transactions itself.
...
* A statement executed outside of a transaction is not committed. The work is discarded when the connection closes.
