...
The link:{jdbidocs}/core/statement/SqlLogger.html[SqlLogger^] interface is called before and after executing each statement,
and given the current `StatementContext`, to log any relevant information desired.

[NOTE]
The time information in `StatementContext` is based on the `Clock` set in
the link:{jdbidocs}/core/Time.html[Time^] configuration.
