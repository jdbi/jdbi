...
Call `one()` when the result contains exactly one row. Throws an exception if
the result has zero or multiple rows.
...
Call `findOne()` when the result contains zero or one row. Throws an exception
...
Call `first()` when the result contains at least one row. Throws an exception
if the result has zero rows.
