...
numeric types, `String`, `UUID`, enums, and the `java.time` types `LocalDate`,
...
[NOTE]
Plugins like `PostgresPlugin` and `H2DatabasePlugin` automatically register additional array element types for their respective databases.
For example, `PostgresPlugin` registers `Duration` and `Period` (bound as `interval`), and binds the date-bearing `java.time` element types as text literals that cover the full Postgres date range, including BC dates.
