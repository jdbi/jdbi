...
<2> Configuring annotations may target types, methods, or both.
...
However, configurers are free to apply different configuration as appropriate
at the type and method level.
...
annotation to our configuring annotation (yo dawg), to tell Jdbi which
`Configurer` to use for this annotation:
