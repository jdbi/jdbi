...
Jdbi can bind/map Java arrays and collections to/from SQL arrays:
...
with Jdbi. An array type that is directly supported by your JDBC driver can
be registered using:
...
Note that plugins like `PostgresPlugin` and `H2DatabasePlugin` automatically
...
jdbi.registerArrayType(new UserArrayType());
----
