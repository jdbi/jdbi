...
===== ImmutableMapper
link:{jdbidocs}/mapper/reflect/ImmutableMapper.html[ImmutableMapper^] uses reflection to map database columns to immutables,
created by the link:immutables.github.io[Immutables Api] or any other Api that creates the Immutable Implementation analog to this api
[source,java]
----
@Value.Immutable
...
Register a immutable mapper for your mapped interface using the factory() method:

----
handle.registerRowMapper(ImmutableMapper.factory(Sample.class));

List<Sample> samples = handle
        .createQuery("select id, name, value_int from sample")
        .mapTo(Sample.class)
        .list();
----

Field mappers can be configured with a column name prefix for each mapped field.
This can help to disambiguate mapping joins, e.g.
when two mapped classes have identical property names (like id or name):

----
handle.registerRowMapper(ImmutableMapper.factory(Contact.class, "c"));
handle.registerRowMapper(ImmutableMapper.factory(Phone.class, "p"));
handle.registerRowMapper(JoinRowMapper.forTypes(Contact.class, Phone.class);
List<JoinRow> contactPhones = handle.select("select " +
        "c.id cid, c.name cname, " +
        "p.id pid, p.name pname, p.number pnumber " +
...
Nested immutable-mapped types can be mapped using the `@Nested` annotation:
