...
Use `@Register___Mapper` annotations on SQL Objects to declaratively register
mappers.

If a row mapper exists for the element type, use `@RegisterRowMapper`:
...
Any mapper class used with this annotation must meet a few criteria:
...
Use `@RegisterRowMapperFactory` to register a `RowMapperFactory`. The factory
class must be public, and have a public, no-argument constructor.
