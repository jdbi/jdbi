...
* Multiple classes follow a similar coding pattern, and a row mapper could be
  implemented in a generalized format. For example, Jdbi provides a generalized
  <<BeanMapper>>, which maps result set columns to bean properties.
...
type `Pair<Foo,Bar>`, it would return `Foo.class`.
...
Note that we are creating an anonymous inner class that extends from
`GenericType<T>`. At runtime, Jdbi inspects the generic type signature and
extracts the `T` parameter for you.
