...
As a special case, you may provide a `Consumer<T>`, `Consumer<Iterator<T>>`, or `Consumer<Stream<T>>` argument in addition to other bound parameters.
When consuming `<T>`, the provided consumer is executed once for each row in the result set.
Otherwise, the consumer is executed once with the `Iterator` or `Stream` holding the results.
The static type of parameter T determines the mapped row type.
