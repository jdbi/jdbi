...
When mapping a value `@Nested Optional<MyBean>` then it will be `Optional.empty()` when all columns in the result set are null,
or when `MyBean` itself would be null (e.g. because `@PropagateNull` annotation is used on a `MyBean` field that was null).
