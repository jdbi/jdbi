...
* https://jdbi.org/releases/3.54.0[Release 3.54.0 - 2026-07-01^]
* https://jdbi.org/releases/3.53.0[Release 3.53.0 - 2026-04-27^] - *Fixes https://github.com/jdbi/jdbi/security/advisories/GHSA-mggx-p7jf-jgw4[GHSA-mggx-p7jf-jgw4] *
* https://jdbi.org/releases/3.52.1[Release 3.52.1 - 2026-04-09^]
* https://jdbi.org/releases/3.52.0[Release 3.52.0 - 2026-03-21^]
...
jdbi-mysql::
Support for https://www.mysql.com/[MySQL^] data types.
jdbi-sqlite::
...
As we can not please everyone equally, there is the link:{jdbidocs}/core/statement/SqlStatements.html#setScriptStatementsNeedSemicolon(boolean)[setScriptStatementsNeedSemicolon()^] switch, which controls this behavior.
...
`OffsetDateTime`, `ZonedDateTime`, `ZoneId` and `ZoneOffset`.
...
When a query joins the same table twice, one mapper per type is not enough: both joined objects have the same type, and a lookup by type alone always finds the mapper registered last.
For this case, register one mapper per column name prefix, and select between them with the `getRow()` overloads that take a prefix:

[source,java,indent=0]
----
List<Folder> folders = handle.createQuery(SELECT_FOLDERS_WITH_PARENT)
    .registerRowMapper(BeanMapper.factory(Folder.class, "f"))
    .registerRowMapper(BeanMapper.factory(Folder.class, "pf")) // <1>
    .reduceRows((Map<Integer, Folder> map, RowView rowView) -> {
        Folder folder = map.computeIfAbsent(
            rowView.getColumn("f_id", Integer.class),
            id -> rowView.getRow(Folder.class, "f")); // <2>

        if (rowView.getColumn("pf_id", Integer.class) != null) {
            folder.setParent(rowView.getRow(Folder.class, "pf"));
        }
    })
    .toList();
----

<1> Two mappers for `Folder`, one for the `f`-prefixed columns and one for the
`pf`-prefixed columns of the self-joined parent folder.
<2> `getRow(Folder.class, "f")` selects the mapper registered with the `"f"`
prefix.

The prefix given to `getRow()` must be equal to the prefix the mapper was registered with.
Only mappers that declare a prefix (`BeanMapper`, `ConstructorMapper`, `FieldMapper`, and `KotlinMapper`, or any custom mapper that implements
link:{jdbidocs}/core/mapper/PrefixedRowMapper.html[PrefixedRowMapper^]) take part in this lookup.
If no mapper with the given prefix is registered for the type, `getRow()` throws an exception rather than fall back to an unprefixed mapper.
...
A `SqlExceptionHandler` receives the raw `SQLException` and the `StatementContext` for the failed statement.
To replace the exception, `throw` a new exception from the handler; if the method returns normally, the next
registered handler is tried. If no handler throws, the original `SQLException` is wrapped and rethrown as an
`UnableToExecuteStatementException`. Multiple handlers are tried in reverse registration order (last registered,
first tried), so more specific handlers should be registered after more general ones.
...
jdbi.getConfig(SqlStatements.class).addExceptionHandler((ex, ctx) -> {
    if ("23505".equals(ex.getSQLState())) {
        throw new DuplicateKeyException("Duplicate key in: " + ctx.getRawSql(), ex);
    }
    // return normally to defer to the next handler (or re-throw the original)
});

// SqlObject annotation
@RegisterSqlExceptionHandler(DuplicateKeyExceptionHandler.class)
public interface UserDao {
    @SqlUpdate("insert into users (id, name) values (:id, :name)")
    void insert(@Bind("id") long id, @Bind("name") String name);
}

public class DuplicateKeyExceptionHandler implements SqlExceptionHandler {
    @Override
    public void handle(SQLException ex, StatementContext ctx) {
        if ("23505".equals(ex.getSQLState())) {
            throw new DuplicateKeyException("User already exists", ex);
        }
    }
}
----
...
Use link:{jdbidocs}/sqlobject/statement/SqlPreflight.html[@SqlPreflight^] to run a literal SQL statement on the same link:{jdbidocs}/core/Handle.html[Handle^], immediately before the annotated method's main statement.
This is useful for setting a scoped or session variable before a query, for example a non-default Postgres trigram similarity threshold:

[source,java,indent=0]
----
@SqlPreflight("select set_config('pg_trgm.word_similarity_threshold', :threshold, true)") // <1>
@SqlQuery("select name from users where name %> :search")
@RegisterFieldMapper(User.class)
List<User> findUser(@Bind("search") String search, @Bind("threshold") String threshold);
----

<1> The method's arguments are bound to the preflight statement, so it can reference `:threshold` just like the main query references `:search`.

The preflight statement binds the method's arguments, respecting binding annotations such as link:{jdbidocs}/sqlobject/customizer/Bind.html[@Bind^] and link:{jdbidocs}/sqlobject/customizer/BindBean.html[@BindBean^].
Arguments that the preflight SQL does not reference are ignored, so a static preflight (one that binds nothing) also works on a method that declares arguments.

The annotation is repeatable--multiple preflight statements run in declaration order--and may be placed on a method or on the SQL Object type, in which case it applies to all of the type's methods (type-level preflights run before method-level ones).

Unlike link:{jdbidocs}/sqlobject/statement/SqlQuery.html[@SqlQuery^] and link:{jdbidocs}/sqlobject/statement/SqlUpdate.html[@SqlUpdate^], the `@SqlPreflight` value is always used as literal SQL; it is not resolved through the configured link:{jdbidocs}/sqlobject/locator/SqlLocator.html[SqlLocator^], so external `.sql` files and link:{jdbidocs}/sqlobject/locator/UseClasspathSqlLocator.html[@UseClasspathSqlLocator^] have no effect on it.

Because the preflight runs on the same connection as the main statement, scoped variables set this way are visible to the query.
The preflight runs during the main statement's setup, so it always executes inside the method's transaction when the method is annotated with link:{jdbidocs}/sqlobject/transaction/Transaction.html[@Transaction^], regardless of the order in which the two annotations are declared.
When connection pooling is in use, prefer transaction-scoped settings (for example `SET LOCAL` in Postgres) together with `@Transaction`, so the setting does not leak to later users of the connection.

NOTE: Jdbi normally rejects a statement when every one of its declared parameters goes unused.
A preflight argument that the *main* statement does not also use is fine as long as the main statement declares at least one parameter of its own.
However, if the main statement declares no parameters at all but the method takes an argument used only by the preflight, annotate the method with link:{jdbidocs}/sqlobject/customizer/AllowUnusedBindings.html[@AllowUnusedBindings^] so the main statement does not fail on that preflight-only argument.
...
=== Mapping Java Time (JSR 310) objects

The link:{jdkdocs}/java.base/java/time/package-summary.html[java.time^] API provides date and time related types since JDK 8. At the same time, Java released JDBC 4.2 which defines mapping of these types to SQL.

Starting with release 3.52.0, Jdbi supports all recent JDBC drivers natively by using the official link:{jdkdocs}/java.sql/java/sql/PreparedStatement.html#setObject(int,java.lang.Object)[PreparedStatement#setObject^] and link:{jdkdocs}/java.sql/java/sql/ResultSet.html#getObject(int)[ResultSet#getObject^] APIs. This affects link:{jdkdocs}/java.base/java/time/LocalDate.html[LocalDate^],
link:{jdkdocs}/java.base/java/time/LocalTime.html[LocalTime^],
link:{jdkdocs}/java.base/java/time/LocalDateTime.html[LocalDateTime^],
link:{jdkdocs}/java.base/java/time/OffsetDateTime.html[OffsetDateTime^] and
link:{jdkdocs}/java.base/java/time/OffsetTime.html[OffsetTime^].

JDBC drivers are not standardized when it comes to mapping data types with timezone onto columns that do not support timezones. Databases all behave differently when mapping data from those columns back onto data types such as link:{jdkdocs}/java.base/java/time/OffsetDateTime.html[OffsetDateTime^] or link:{jdkdocs}/java.base/java/time/ZonedDateTime.html[ZonedDateTime^].

Jdbi used to map these types explicitly to the legacy (pre-JDBC 4.2) types such as link:{jdkdocs}/java.sql/java/sql/Timestamp.html[Timestamp^], link:{jdkdocs}/java.sql/java/sql/Date.html[Date^] and link:{jdkdocs}/java.sql/java/sql/Time.html[Time^]. None of those datatypes support timezones so heuristics were used for data types with timezones.

These heuristics have changed with 3.52.0. If the old behavior is wanted, it is possible to qualify the mapping type with the link:{jdbidocs}/meta/Legacy.html[@Legacy^] annotation:

[source,java,indent=0]
----
var legacyType = QualifiedType.of(OffsetDateTime.class).with(Legacy.class);
try (Update u = h.createUpdate("INSERT INTO time_table (time_col) VALUES (?)")) {
    u.bindByType(0, OffsetDateTime.now(), legacyType);
    u.execute();
}

var result = h.createQuery("SELECT time_col FROM time_table").mapTo(legacyType).list();
----

[source,java,indent=0]
----
interface LegacyDao {
    @SqlUpdate("INSERT INTO time_table (time_col) VALUES (:now)")
    void update(@Legacy OffsetDateTime now);

    @SqlQuery("SELECT time_col FROM time_table")
    @Legacy List<OffsetDateTime> query();
}
----
...
Then, on any link:{jdbidocs}/core/config/Configurable.html[Configurable^] context (like a link:{jdbidocs}/core/Jdbi.html[Jdbi^] or link:{jdbidocs}/core/Handle.html[Handle^]) you can change configuration in a type safe way:
...
[source,java,indent=0]
----
jdbi.installPlugin(new Jackson3Plugin());
// optionally configure your ObjectMapper (recommended)
jdbi.getConfig(Jackson3Config.class).setMapper(myObjectMapper);
...
The plugin configures mappings for link:{jdkdocs}/java.base/java/time/package-summary.html[java.time^] types like link:{jdkdocs}/java.base/java/time/Duration.html[Duration^], link:{jdkdocs}/java.base/java/net/InetAddress.html[InetAddress^], link:{jdkdocs}/java.base/java/util/UUID.html[UUID^], typed enums, and <<postgres-hstore,hstore>>.
...
The *jdbi-mysql* plugin provides enhanced integration with the
https://dev.mysql.com/doc/connector-j/en/[MySQL JDBC Driver^].
...
<dependency>
    <groupId>org.jdbi</groupId>
    <artifactId>jdbi-mysql</artifactId>
    <version>{project_version}</version>
</dependency>
----
...
----
dependencies {
    implementation("org.jdbi:jdbi-mysql:{project_version}")
}
----
...
[source,java,indent=0]
----
Jdbi jdbi = Jdbi.create("jdbc:mysql://host:port/database")
                .installPlugin(new MysqlPlugin());
----

The plugin configures mappings for data types from link:{jdkdocs}/java.base/java/time/package-summary.html[java.time^] that support time zones: link:{jdkdocs}/java.base/java/time/OffsetDateTime.html[OffsetDateTime^], link:{jdkdocs}/java.base/java/time/OffsetTime.html[OffsetTime^], link:{jdkdocs}/java.base/java/time/ZonedDateTime.html[ZonedDateTime^].

[NOTE]
MySQL does not have a data type that manages time zones! Loading this module makes it possible to map the java types directly onto columns. This converts the timezone to the system timezone and then creates a local timestamp (without a timezone). Without the module, the database driver will report an error that it does not support `TIMEZONE_WITH_TIMESTAMP` SQL types.
...
interface.
By calling link:{jdbidocs}/core/config/Configurable.html#setTemplateEngine(org.jdbi.core.statement.TemplateEngine)[Configurable#setTemplateEngine()^] method on the link:{jdbidocs}/core/Jdbi.html[Jdbi^], link:{jdbidocs}/core/Handle.html[Handle^], or any SQL statement like link:{jdbidocs}/core/statement/Update.html[Update^] or link:{jdbidocs}/core/statement/Query.html[Query^], the template engine used to render SQL can be changed:
...
By calling link:{jdbidocs}/core/config/Configurable.html#setSqlParser(org.jdbi.core.statement.SqlParser)[Configurable#setSqlParser()^] method on the link:{jdbidocs}/core/Jdbi.html[Jdbi^], link:{jdbidocs}/core/Handle.html[Handle^], or any SQL statement like link:{jdbidocs}/core/statement/Update.html[Update^] or link:{jdbidocs}/core/statement/Query.html[Query^], the parser used to define named arguments can be changed:
