...
link:{jdbidocs}/sqlobject/statement/SqlUpdate.html[@SqlUpdate^],
link:{jdbidocs}/sqlobject/statement/SqlScripts.html[@SqlScripts^] and
...
SQL Objects are a declarative-style extension to the fluent-style Core APIs. SQL Objects are implemented as an extension to the core API using the <<extension-framework,Extension framework>>.
...
With SQL Object, you declare a public interface as an extension type, add methods for each database
...
All methods except `finalize()` can be overridden by the extension type (e.g. through a interface default method).
...
which renders templates using the https://www.stringtemplate.org/[StringTemplate library^]. See <<StringTemplate 4>>.
