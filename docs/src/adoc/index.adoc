...
Without it, mappers come from the registry by type and prefix, so register `BeanMapper.factory(Contact.class)` and `BeanMapper.factory(Phone.class, "phone")` instead.
...
Root objects are returned in the order of their first row, and only keys in the root key column become roots.
...
public class FolderDocReducer extends JoinRowReducer<Folder> {
    public FolderDocReducer() {
