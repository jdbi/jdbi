...
Each module that defines a `Jdbi` instance keeps all bindings private and exposes only the actual `Jdbi` object itself. This allows the installation of multiple modules where each definition is completely independent. Sometimes it is useful to attach additional objects and expose them using the same annotations. The most common use cases are data access objects.
