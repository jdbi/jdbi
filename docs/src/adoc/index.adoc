...
When connection pooling is in use, prefer transaction-scoped settings (for example `SET LOCAL` in Postgres) together with link:{jdbidocs}/sqlobject/transaction/Transaction.html[@Transaction^], so the setting does not leak to later users of the connection.
