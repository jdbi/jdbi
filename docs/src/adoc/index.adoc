...
// TODO move the stream stuff to `ResultIterable`?
...
The link:{jdbidocs}/core/generic/GenericTypes.html[GenericTypes^] utility class
provides several methods for helping to interpret Java generic types.

The method `GenericTypes.getErasedType` accepts a `java.lang.reflect.Type` and
returns the raw `java.lang.Class` for that type. So if `type` were the generic
type `Pair<Foo,Bar>`, it would return `Pair.class`.
...
The `GenericTypes.resolveType` method accepts a generic type parameter to
resolve, and a context type in which to resolve it.

In the first line, `Pair.class.getTypeParameters()[0]` gives the type variable
`L`. So if `type` was `Pair<Foo, Bar>`, the call to `resolveType` means "resolve
the type variable `L` in the context of `Pair<Foo, Bar>`, which resolves to the
left parameter, `Foo`.

Likewise in the second line, `Pair.class.getTypeParameters()[0]` gives the type
variable `L`, so the `resolveType` call returns `Bar`.
...
But how can we map to `Pair<String, String>`? Java doesn't provide a type-safe
way to pass around generic types.

For this, Jdbi provides a `GenericType<T>` class:
...
Note that the expression `new GenericType<Pair<String, String>>() {}` creates an
anonymous inner class that extends from `GenericType<T>`. At runtime, Jdbi
inspects the generic type signature and extracts the `T` parameter for you.
...
The method `GenericTypes.getErasedType` accepts a `java.lang.reflect.Type` and
returns the raw `java.lang.Class` for that type. So if `type` were the generic
type `Optional<String>`, it would return `Optional.class`.

Next, we extract the `T` generic parameter from the mapped type:

[source,java]
----
Type embeddedType = GenericTypes.resolveType(Optional.class.getTypeParameters()[0], type);
----
...
`T`. So if `type` was `Optional<String>`, the call to `resolveType` means
"resolve the type variable `T` in the context of `Optional<String>`, which
resolves to the type `Foo`.
...
[source,java]
----
ColumnMapper<?> embeddedMapper = config.get(ColumnMappers.class)
...
ColumnMapper<?> optionalMapper = (rs, col, ctx) ->
    Optional.ofNullable(embeddedMapper.map(rs, col, ctx));
...
But how can we map to `Optional<String>`? Java doesn't provide a type-safe way
to pass around generic types.
...
Note that the expression `new GenericType<Optional<String>>() {}` creates an
anonymous inner class that extends from `GenericType<T>`. At runtime, Jdbi
inspects the generic type signature and extracts the `T` parameter for you.
...
* Lombok's `@AllArgsConstructor` generates `@ConstructorProperties` on
  constructor, so constructor mappers work well with data classes that have it.
