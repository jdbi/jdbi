...
===== Executing Operations in Chunks
You can set chunk size with `PreparedBatch.setBatchChunkSize()` which chunks the operations into your preferred size. See below, instead of the 5000 insert operations being executed at once, they will be executed 1000 operations at a time:

[source,java,indent=0]
----
PreparedBatch batch = handle.prepareBatch("INSERT INTO \"user\" (id, \"name\") VALUES(:id, :name)");

batch.setBatchChunkSize(1000);
