...
When the nested attribute is `@Nullable` or wrapped in `Optional` then then the attribute will automatically be `null` or `Optional.empty()` if all mapped nested fields are `null` in the result set.
