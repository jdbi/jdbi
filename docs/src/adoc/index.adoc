...
`Jdbi` runs on all Java versions 8 or later. 11 or later is requied to build.
...
By using the TypeResolvingCodecFactory from the jdbi-guava module , it is possible to provide type safe resolution for subclasses. This is necessary to e.g. map Auto Value classes to database columns.
