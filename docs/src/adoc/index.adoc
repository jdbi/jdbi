...
jdbi3-spring::
    Provides a factory bean to set up Jdbi singleton. Integration with Springs transactions
...
This plugin Provides a factory bean to set up a Jdbi singleton for convenience and
integrates with Springs transactions.
...
When using Jdbi with Spring you most likely want support for Springs transaction
management system.

It should be noted, that you need not install any plugin if you
don't want to introduce dependencies in your system,
since there is an out-of-the-box support for transactions via
`org.springframework.jdbc.datasource.TransactionAwareDataSourceProxy`.

However, this approach has potential drawbacks since Jdbi does not know about Spring
and vice versa. It is possible for the underlying connection of a `Handle` to
behave differently than expected or even be closed.

This is why this plugin introduces APIs to avoid relying on `TransactionAwareDataSourceProxy`.
If you are familiar with Spring you will notice the similarities to its existing APIs.
That is - there is a low-level way of doing things via `JdbiUtil` or a high-level one
via `JdbiTemplate`/`JdbiOperations`

[CAUTION]
As of now the high-level APIs are considered `@Beta` and marked as such.

[TIP]
Since the high-level approach follows the exact same signatures
of `Jdbi` itself, you could easily
replace your injected Jdbi dependency, if you decide to use "plain" `Jdbi`

===== JdbiUtil

Similar to Springs `DataSourceUtils` you can use the `JdbiUtil` static methods to
manage (obtain and close) a handle by yourself
...
Since you probably don't want to manage the `Handle` yourself it is preferred to
use a much more jdbi-ish and spring-ish way:

===== JdbiTemplate

Have a `JdbiTemplate` or `JdbiOperations` instance injected by your spring container
and use Jdbi the way god intended. Here is how in Java config:
...
public class Config {
    ...
    public PlatformTransactionManager transactionManager(Jdbi jdbi) {
        return new JdbiTransactionManager(jdbi); // <1>
    }

    @Bean
    public JdbiOperations jdbiOperations(Jdbi jdbi) {
        return new JdbiTemplate(jdbi); // <2>
    }
    ...

}
----
<1> Set up a transaction manager. Note that this is a `JdbiTransactionManager`
<2> Create a factory method for your `JdbiTemplate` bean. Use the `JdbiOperations`
abstraction if you prefer.

That's great - but where does the `Jdbi` bean come from?

==== JdbiFactoryBean

Well - either you set up a bean of type `Jdbi` yourself or
you could use the `JdbiFactoryBean` provided by this plugin. Lets see how:
...
Easy, right? Now why would you do that and not set up `Jdbi` directly?
There are 3 compelling reasons:

1. Auto installation of `JdbiPlugin` beans
2. Auto registration of `@Component`s (beans) that implement `RowMapper<T>` and
`ColumnMapper<T>` interfaces
3. Possibility to make global defines and plugin auto detection configurable in spring

The latter are especially useful if you use xml config. E.g.
...
[TIP]
`autoInstallPlugins` and auto installation of the first mentioned point
are not the same.
The former installs everything available on the classpath via service loader
and the latter requires plugins to be a Spring `@Bean`.

[CAUTION]
The auto registration of mappers does not work if you directly expose
lambdas as `@Beans`. In this case Jdbi is unable to get the mapped type right.
So make sure you use classes implementing the respective interfaces.
