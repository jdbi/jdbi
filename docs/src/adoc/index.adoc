...
* Template engines take an SQL template, and produce a parseable SQL statement.
* Out of the box, Jdbi processes `<name>`-style by substituting in the string
  value of the named attribute. This is accomplished by
  `DefinedAttributeTemplateEngine`.
* Other template engines can be configured, e.g. `StringTemplateEngine`, which
  uses String Template v4.
* Configure a different template engine by calling `setTemplateEngine()` on a
  `Jdbi`, `Handle`, or on a SQL statement like `Update` or `Query`.

=== SqlParser

TODO:

* Parses a SQL string and produces a `ParsedSql` object, which includes the
  final SQL string (using positional-only parameters) and the parameters parsed
  out of the input string.
* Depending on your database vendor (Oracle?), you can use hash prefix named
  parameters. Configure HashPrefixSqlParser as the default
  SqlParser.
* Out of the box, the default parser (`ColonPrefixSqlParser`) looks for named
  parameters using a colon prefix, e.g. `:name`.
* Configure a different parser (e.g. `HashPrefixSqlParser`) by calling
  `setSqlParser()` on a `Jdbi`, `Handle`, or SQL statement such as `Update` or
  `Query`.
