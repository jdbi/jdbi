...
To use StringTemplate format in SQL statements, set the statement rewriter
to `StringTemplateStatementRewriter`.
...
List<Account> accounts = handle.createQuery(sql)
      .setStatementRewriter(new StringTemplateStatementRewriter())
      .define("sort", true)
...
In SQL Objects, the `@UseStringTemplateStatementRewriter` annotation sets the
