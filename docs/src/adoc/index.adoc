...
The link:{jdbidocs}/sqlobject/customizer/DefineList.html[@DefineList^] annotation converts an array or link:{jdkdocs}/java.base/java/util/List.html[List^] argument into a comma-separated string and defines it as a template attribute.
This is useful for dynamically specifying column names or other SQL fragments that cannot be used as bound parameters.

`@DefineList` uses a template placeholder (the `<name>` notation) similar to link:{jdbidocs}/sqlobject/customizer/Define.html[@Define^]:

[source,java,indent=0]
----
public interface UserDao {
    @SqlQuery("select <columns> from <table> where id = :id")
    Something findById(@DefineList("columns") List<String> columns,
                       @Define("table") String table,
                       @Bind("id") long id);
}
----

It can be combined with link:{jdbidocs}/sqlobject/customizer/BindList.html[@BindList^] to dynamically define both the column list and the values:
...
[WARNING]
Unlike bound parameters, defined attributes are inserted directly into the SQL statement without escaping or sanitization. **Passing untrusted input through `@DefineList` may make your application vulnerable to SQL injection.** Only use values that are under application control such as column or table names defined in code.

[NOTE]
An empty list or `null` elements in the list will result in an link:{jdkdocs}/java.base/java/lang/IllegalArgumentException.html[IllegalArgumentException^].
