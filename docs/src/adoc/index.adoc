...
using the `stmt.defineNamedBindings()` or `@DefineNamedBindings` customizers, which will
define a boolean indicating whether the value is present and not a `NullArgument` (bind of a `null`).
